...
func TestAllocRunner_Lifecycle_Restart(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			// TODO: currently failing because we get an error from trying to
			// restart the dead oneshot tasks, and we bubble those errors up
			// to the caller
			expectedAfter: map[string]structs.TaskState{
...

		{
			...
			// TODO: currently fails because poststart-sidecar isn't restarted,
			// but should it be?
			expectedAfter: map[string]structs.TaskState{
				"main":              structs.TaskState{State: "running", Restarts: 1},
				"prestart-oneshot":  structs.TaskState{State: "dead", Restarts: 0},
